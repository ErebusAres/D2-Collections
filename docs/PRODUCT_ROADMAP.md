...
- **Loot management:** Gear now has a tall Loot workspace that defaults to all first-observed physical items in a rolling seven-day period, preserves tagged/reviewed entries during that period, and supports 1/3/7/14/30-day views. Loot is presented as compact icon cards with Power on weapons and armor, plus weapon-only source-backed PvE/PvP/overall percentages and Excellent/Strong/Mixed/Weak/Poor tiers. The detail surface shows Bungie stats, tracker values when returned, masterwork, active perks, evidence basis, confidence, and reasons; armor deliberately has no rating. Physical-item tags remain account-private and “acquired” remains explicitly the first Guardian Nexus observation. The schema-v3 evaluator uses actual perk-column weighting from the default DIM Voltron community wishlist, covers all 17 weapon types with explicitly lower-confidence fallback profiles, and keeps unreviewed/incomplete evidence distinct from a low score.
