...
- **Loot management:** Gear now has a tall Loot workspace that defaults to all first-observed physical items in a rolling seven-day period, preserves tagged/reviewed entries during that period, and supports 1/3/7/14/30-day views. Loot is presented as compact icon cards with Power on weapons and armor, plus weapon-only source-backed PvE/PvP/overall percentages. The detail surface shows Bungie stats, tracker values when returned, masterwork and active perks; armor deliberately has no rating. Physical-item tags remain account-private and “acquired” remains explicitly the first Guardian Nexus observation. The schema-v2 evaluator is generated from the default DIM community wishlist and keeps unreviewed/incomplete states distinct from a low score.
...
- [x] Shared icon-card recent-loot components, global physical-item tags, keyboard tag shortcuts, and a versioned DIM-community evaluation model with separate PvE/PvP coverage and honest unknown states.
