...
- **Loot management:** Gear now has a tall Loot workspace that defaults to all first-observed physical items in a rolling seven-day period, preserves tagged/reviewed entries during that period, and supports 1/3/7/14/30-day views. Loot is presented as compact icon cards with Power on weapons and armor, plus weapon-only source-backed PvE/PvP/overall percentages and Excellent/Strong/Mixed/Weak/Poor tiers. The detail surface shows Bungie stats, tracker values when returned, masterwork, active perks, evidence basis, confidence, and reasons; armor deliberately has no rating. Fireteam's private Recent Loot retains 12, 24, or 48 mixed entries but shows one responsive row at a time with previous/next paging. Its weapon cards show the rating system, while catalyst cards retain visible capacity alongside gear and use a green check/100% Masterworked state. Physical-item tags remain account-private and “acquired” remains explicitly the first Guardian Nexus observation. The schema-v3 evaluator uses actual perk-column weighting from the default DIM Voltron community wishlist, covers all 17 weapon types with explicitly lower-confidence fallback profiles, and keeps unreviewed/incomplete evidence distinct from a low score.
- **Support diagnostics:** `/support` is intentionally outside normal Guardian bootstrap and navigation. It checks browser/build state, the current server session and D1 relationship, OAuth resolution, raw Bungie memberships, every linked Profiles/Characters response, verified membership selection, and the normal profile/account normalization path. Reports are sanitized, read-only, current-session-only, and copied only by explicit action.
...
- [x] Shared icon-card recent-loot components, global physical-item tags, keyboard tag shortcuts, and a versioned DIM-community roll evaluation model with weighted perk columns, separate PvE/PvP scores, quality tiers, source/confidence explanations, all-type fallback coverage, and honest unknown states.
- [x] Fireteam Recent Loot capacity controls with a 24-card default, a 48-card option, and catalyst observations rendered in the same icon-card system as gear.
