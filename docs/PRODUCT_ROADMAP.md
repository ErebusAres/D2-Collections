...
- **Loadout workspace:** The selected Guardian's live equipped configuration appears above saved loadouts using the same item, socket, subclass, and Artifact presentation. Bungie gaps remain partial or unavailable; loadout cards can minimize to headers with item icons and a numbered floating rail jumps between sections.
