...
- **Loot management:** Gear keeps its rolling first-observed physical-item workspace and source-backed weapon ratings. Fireteam uses a private 30-day item-event timeline for observed weapon, armor, catalyst-found, catalyst-completed, and stackable inventory-gain changes. Its responsive one-row pager flows newest-left to oldest-right across the full retained history; catalysts are events rather than pinned state, and rapid identical material gains coalesce into `×N` cards. A first visit silently establishes catalyst/material baselines so existing holdings are not mislabeled as new. Physical-item tags remain private and event times are snapshot-observation times, not claimed pickup times.
...
- [x] Fireteam item-event timeline with newest-left paging, durable 30-day history, chronological catalyst acquisition/completion, material quantity deltas, and rapid duplicate coalescing.
