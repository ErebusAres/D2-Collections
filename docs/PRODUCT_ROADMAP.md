...
- **Loot management:** Gear keeps its rolling first-observed physical-item workspace and source-backed weapon roll matching. Fireteam uses a private 30-day item-event timeline for observed weapon, armor, catalyst-found, catalyst-completed, and stackable inventory-gain changes. Its responsive one-row pager flows newest-left to oldest-right across the retained 200-card history; catalysts are events rather than pinned state, and rapid identical material gains coalesce into `×N` cards without discarding their underlying observations. A first visit silently establishes every item baseline so existing holdings are not mislabeled as new. Observation continues across signed-in Guardian Nexus pages—even when the Fireteam row is hidden—while the site is open. Physical-item tags remain private and event times are snapshot-observation times, not claimed pickup times.
...
- [x] Shared icon-card recent-loot components, global physical-item tags, keyboard tag shortcuts, and a versioned DIM-community roll evaluation model with preserved trait pairings, equal source-faithful column weights, separate PvE/PvP scores, quality tiers, source/confidence explanations, all-type fallback coverage, and honest unknown states.
- [x] Fireteam item-event timeline with newest-left paging, silent all-item baseline, failure-safe inventory snapshots, retry-stable event identities, durable 30-day history, chronological catalyst acquisition/completion, material quantity deltas, and rapid presentation coalescing.
