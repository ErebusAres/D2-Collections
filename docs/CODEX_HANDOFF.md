...
PR #149 (`414a2292a939e48b3e721684a00bca2791d9ea6c`) made Analyze a lightweight durable request and deployed successfully through production workflow `35944990223`. Migration `0038_cleanup_analysis_cache.sql` stores complete analyses by account and canonical settings hash, including timestamps, expiry, refresh lease, requested time, and the last recoverable error. Analyze saves preferences, enqueues the requested settings, and immediately returns either the exact saved result, the most recent complete fallback, or an honest queued state. It no longer performs the expensive rebuild on the browser response path. Ten-minute freshness avoids redundant work while older results remain usable and explicitly stale; explicit searches retain only the eight most recent settings variants per account.

Signed-in production acceptance confirmed that `POST /api/v1/me/cleanup/analyze` now returns the queued state immediately without a 1102. The first scheduled full rebuild still failed to publish a result, demonstrating that moving the same Bungie/full-manifest workload to cron changed request isolation but did not reduce its CPU cost. A separate incident during the same acceptance pass was `GET /api/v1/me/gear`, reference/Ray `a3fe31ee7c42276c`; do not attribute that second failure to Cleanup.

Branch `codex/cleanup-saved-baseline` removes the heavy rebuild from scheduled Cleanup work. It builds and retains a review-only analysis from the current physical gear already maintained in `recent_item_observations`, including saved build references and Cleanup marks/dismissals. It refreshes that durable result at most every ten minutes as the independent Recent Items observer advances. Every saved-snapshot recommendation is non-actionable and explicitly says live protection verification is pending; preference-source comparisons and any capability comparison unsupported by saved fields remain deferred and labeled rather than guessed. Explicit mutations continue to perform their existing live revalidation, but the baseline cannot offer those actions. This prevents a recurring background 1102 loop while preserving useful prior results and incrementally updating them from successful observations.

PR #150 (`1a3bc724284b8e3e7f2202fcef1687cf50cb4c15`) passed and deployed through workflow `35954062771`. Production continued to return a healthy queued response without a Cleanup 1102, but the account's requested variant did not publish after several scheduler cycles. The follow-up `codex/cleanup-request-refresh` therefore starts the same D1-only leased refresh with `waitUntil` whenever a Cleanup GET or Analyze POST sees stale work. The HTTP response still returns first, concurrent page polling cannot duplicate the claimed lease, and cron remains a fallback instead of the only path through the global one-job-per-minute queue.

PR #151 (`837f24d05cb5656a2c97ad744f7f110979112996`) passed and deployed through workflow `35955044373`. Request-owned execution removed the global queue delay but still did not publish the account's 1,295-item result, confirming that transferring and parsing every full saved gear record in one Worker turn remained too large. PR #152 (`519823c673c96458cabb6bb675b92ae3fb417d27`) bounds each refresh to 200 gear observations, cycles through deterministic item-hash-ordered pages every ten minutes, merges prior review recommendations with the current page, and retains only candidate/keeper gear in the cached response. A 30-second lease is sufficient for the bounded job and recovers quickly after any killed predecessor. Production workflow `35955903818` passed, and signed-in acceptance completed page 7/7 as a current saved analysis with no alert or Cleanup 1102. The UI reports the current pass size, page, and total observed inventory without implying unprocessed pages are complete; all incremental recommendations remain non-actionable.
...
PR #149 validation passed the complete `pnpm run audit`; migration application and both deployments passed. The saved-baseline follow-up also passes the complete audit: archive/source/CSS boundaries across 45 stylesheets, ESLint, every TypeScript target, 43 domain tests, 300 API tests, 336 Web tests, tooling/Python tests, API and Web production builds, and performance budgets at 373,604 bytes entry JavaScript (115,379 gzip) and 36,321 bytes CSS. Follow-up PR, merge, deployment, and signed-in production verification remain pending at this checkpoint.
...
- Released through PR #154 and squash merge `40a3265`; production workflow `36078758443` applied migration `0039`, deployed the API and web, and completed successfully.
- Signed-in inspection-only production verification confirmed Guardian services connected with no incident banner, the cosmetic-sync warning cleared after the background refresh, the weapon picker listed 182 verified shaders, and the Warlock picker listed 31 owned styles with honest `n/5` availability. No shader/style was selected or applied, and no gear was tagged, pulled, moved, equipped, or otherwise changed.

## 2026-09-25 Gear route 1102 follow-up

- Production incident `a4064ea6ccd0b4d3` / Cloudflare Ray `a4064ea6ccd0b4d3-BNA` confirmed `GET /api/v1/me/gear` could still exceed Worker resource limits. A request one minute earlier succeeded in 1.1 seconds; the failing request terminated at 1.64 seconds. The interactive route was still fetching the full Bungie profile, parsing the large Gear manifest, and normalizing all 1,295 physical items every 60 seconds.
- Current implementation moves that work behind the existing Recent Loot background refresh. Migration `0040_guardian_gear_cache.sql` stores the already-normalized account-wide `GearData` snapshot. Interactive Gear reads the durable snapshot, overlays current local tags/dismissals and Cleanup marks, adapts the selected character, and returns stale-but-usable data with an explicit warning while refresh is delayed.
- A missing cache queues and immediately prioritizes the existing background refresh, returning a structured `gear_snapshot_pending` response rather than a Cloudflare HTML 1102. The cache write happens before cosmetic/timeline derivation so later optional work cannot prevent the core Gear snapshot from becoming available.
- Initial production bootstrap exposed two ordering defects before completion was claimed: resetting the shared Recent Loot timestamp surfaced a 1969 Cleanup date, and the job loaded the full Collection/companion data before committing Gear. The follow-up removes that timestamp mutation, prioritizes memberships missing `guardian_gear_cache` directly in the scheduler query, and commits Gear before loading Collection/companion enrichment.
- Two lease-expiry retries still could not populate the cache, confirming the 11.1 MB Gear artifact itself remained over the runtime budget. The final follow-up generates and uses `gear-runtime-manifest.json`: physical gear presentation plus armor/weapon roll plugs, but no cosmetic catalog, appearance socket metadata, cleanup capability graph, or static perk-pool duplication. Cosmetic cache refresh is deliberately skipped on this compact path so existing verified appearance options are preserved rather than overwritten by partial data.
- Production inspection then showed the compact artifact deployed but the Recent Loot scheduler still did not service this account while the separate Cleanup worker advanced. A final bounded fallback now performs the compact build only when `guardian_gear_cache` has no row; every subsequent Gear poll remains a lightweight snapshot read. The background request remains queued for normal refreshes.
- Releases are live in dependency order: PR #156 / `fc5d516` / workflow `36083621786` added the durable Gear snapshot; PR #157 / `b35f798` / workflow `36084220274` corrected refresh ordering and missing-cache priority; PR #158 / `19f809c` / workflow `36085302386` deployed the compact runtime manifest; PR #159 / `6a2da25` / workflow `36085922730` added the one-time compact bootstrap for accounts the global queue has not reached. Every production workflow passed its migrations, audit, API deployment, and Web deployment.
- Signed-in, inspection-only production acceptance after PR #159 confirmed the incident banner cleared, the connection control reported `Guardian services connected`, and Gear returned all 1,295 observed items (727 armor, 568 weapons, 1,233 in the Vault) with a current 05:45 AM route freshness time. Five consecutive authenticated reloads completed in 1.49-1.71 seconds; every load retained the full Armor/Weapons counts with no over-capacity or resource-limit state. Cleanup retained its bounded 200-item page and prior recommendations. No gear, tags, loadouts, shaders, ornaments, or inventory state were changed.
- The original timestamp reset left one already-stored Recent Loot refresh time at the Unix epoch, so Cleanup could truthfully use current Gear while still displaying a 1969 inventory date. The final follow-up treats pre-2020 observation timestamps as invalid and falls back to the independently current durable Gear snapshot timestamp; it does not rewrite or invent acquisition times.
- Full `pnpm run audit` passes outside the known OneDrive/esbuild sandbox restriction: archive/source/CSS boundaries, ESLint, every TypeScript target, 43 domain tests, 303 API tests (including new cache parsing and current-state overlay coverage), 336 web tests, 7 Node tooling tests, 29 Python manifest tests, API/Web production builds, and performance budgets of 373,604 bytes entry JavaScript (115,379 gzip) and 36,321 bytes CSS. The timestamp follow-up adds one API regression (304 API tests total). Remaining operational evidence: Cloudflare CPU/log headroom and longer-duration production monitoring; live verification remains inspection-only.

## 2026-09-25 D1 daily-limit hardening

- Cloudflare's verified alert reported that `guardian-nexus-v1` crossed 75% of the Free-plan daily D1 row-read allowance. Production Query Insights for the preceding 24 hours showed 323,000 queries, 4.79 million rows read, and 8,870 rows written. This was normal application traffic rather than Codex browser testing.
- The largest avoidable reads were full account scans: `gear_item_state` used 1.29 million rows across 854 requests; the Recent Loot `COUNT(*)`/`MAX(updated_at)` summary used 871,610 rows across 552 requests; and the watcher seen-set query used 445,640 rows across 326 requests. The watcher also issued 295,136 redundant `INSERT OR IGNORE` statements during the same period.
- Normal Gear reads now hydrate the durable snapshot with only rows changed since that snapshot plus persistent tag/dismissal overrides. Snapshot state is preserved when no override exists. Migration `0041_d1_read_efficiency.sql` adds the matching partial index and an index for the Recent Loot timeline's actual sort order.
- Recent Loot summary reads now use an indexed existence probe plus the one-row refresh-state record rather than aggregating every observation. The observation load selects only fields used by diffing instead of `SELECT *`.
- Background Recent Loot observation and Loot Watcher inventory passes now run on a five-minute cadence, matching the product's declared freshness interval. Explicit watcher setting changes still queue an immediate run, and saved timelines remain available between passes.
- Loot Watcher no longer reads or rewrites the entire `loot_watcher_seen_items` inventory set. It derives newness from the existing per-item `first_seen_at` state relative to the watcher's last successful run; an account's first successful pass remains a silent baseline. This structurally removes roughly 295,000 SQL statements per day at the measured traffic level, before accounting for the cadence reduction.
- Verification completed before release: 306 API tests across 45 files, API typecheck, repository ESLint/typecheck, all 42 migrations on a fresh local D1 database, and `git diff --check`. GitHub's Linux release audit then passed in full.
- Operational references: Cloudflare D1 pricing explains rows-scanned billing and the five-million-row Free limit; Query Insights exposes query text and row counts; Cloudflare recommends indexes to reduce scanned rows. Future diagnostics should compare a short post-deploy window before treating the rolling 24-hour total as evidence of regression.
- Released through PR #161 and squash merge `679d193`; production workflow `36206747540` applied migration `0041`, deployed API Worker version `636b252c-b4f1-47e9-97d7-5814746b00cd`, and deployed the Web application successfully.
- Signed-in, inspection-only production acceptance confirmed `Guardian services connected`, all 1,295 Gear items (727 armor, 568 weapons, 1,233 in the Vault), and the separate Recent Weapons, Recent Armor, and Recent Loot rows with ratings and real gear-tier diamonds. Repeated reloads produced no over-capacity state.
- In the first short post-deploy Query Insights comparison, total queries in the 30-minute window increased from about 4,000 to 5,000 while every retired hot-query count remained unchanged: 4,127 redundant watcher inserts, 14 watcher seen-set reads, 6 full observation scans, and 13 full Gear-state scans. That confirms the deployed Worker stopped issuing those statements. Recheck the daily total after the rolling 24-hour window contains only the new release before treating the long-term reduction as fully measured.

## 2026-09-25 notification service-alert lifecycle repair

- Root cause: Bungie global service alerts had no end time, critical alerts were intentionally non-dismissible, and history deduplicated only exact notification IDs. Repeated observations therefore remained active indefinitely and filled the drawer/history with identical `Destiny service alert` rows.
- The notification read model now stacks Bungie global-alert occurrences by normalized title/subtitle before pagination. The newest occurrence represents the stack and carries its occurrence count and first-seen timestamp in metadata; both the drawer and full history label the stack count.
- The maintenance pass now reconciles saved alerts against the current successful Bungie global-alert response. Missing alerts are expired only when that provider is confirmed live; a provider outage preserves last-known alerts instead of falsely resolving them. Resolved service-alert occurrences are retained for seven days, then removed with cascading per-user state cleanup.
- Production verification exposed that Bungie's successful global-alert response still contained the same ten-day-old incident. Source normalization now treats provider records older than 48 hours as stale even if Bungie retains them, allowing reconciliation to expire and purge the obsolete stack without hiding a current incident.
- Validation before release: 308 API tests across 45 files, API and Web typechecks, repository ESLint, `git diff --check`, and a direct Node SQLite execution of the production window-function query. The SQLite check collapsed 13 service-alert rows into one stack without collapsing an unrelated notification. Local Web Vitest remains blocked by the known OneDrive/esbuild sandbox traversal restriction; Linux CI is the full browser-test release gate.
