...
Fireteam loot lines and every Recent Loot icon card use one shared rarity-colored diamond. The diamond contains the Destiny manifest tier number (Exotic 6, Legendary/Superior 5, Rare 4, Common 3, Uncommon/Basic 2, Currency 1, unknown 0) as a dark hollow-style numeral for rapid scanning. It overlays the item thumbnail on Fireteam Recent Loot and all three Gear history rows; Fireteam Activity keeps the same diamond immediately before its small thumbnail and rarity-colored name. Missing manifest rarity stays explicit tier 0 instead of being invented as Common or Legendary.
...
- Current implementation branch: `codex/recent-loot-tiers-live-handoff`, a documentation-only checkpoint after the complete PR #101 tier-diamond release. The implementation is live on `main`.
...
- Recent Loot tier-diamond validation on 2026-08-08: signed-in production inspection established the pre-fix defect with 14 visible Fireteam Recent Loot cards and zero tier labels while Fireteam Activity already displayed them. The implementation maps every DestinyTierType value and both site aliases through one renderer, overlays it on every recent icon card, and changes unresolved API rarity fallbacks from false Common/Legendary values to Unknown. Full `pnpm run audit` passes archive/source/CSS boundaries across 40 stylesheets, ESLint, every TypeScript target, 24 domain tests, 195 API tests, 267 web tests, tooling/Python tests, production builds, and performance budgets. Output is 365,322 bytes entry JavaScript (112,611 bytes gzip) and 33,043 bytes CSS. PR #101 merged as `e71048769d63e06f1b2d044ccd6f5b2489908775`; production workflow `31288697707` refreshed manifests/ratings, reran the audit, applied migrations, and deployed the API and web successfully. Signed-in post-deploy QA confirmed diamonds on 14/14 visible Fireteam Recent Loot cards, 9/9 Gear Recent Weapons, 7/7 Gear Recent Armor, and 17/17 Gear Recent Loot cards. Real Legendary cards displayed purple tier 5, Common cards displayed tier 3, Fireteam Activity retained tier 5 diamonds, and every inspected item image loaded.
