...
export function CleanupWorkspace({ onTag, analysisRequest = 0 }: { onTag: (item: LootItem, tag?: GearTag) => void; analysisRequest?: number }) {
  ...
  return <section className={styles.workspace}>
    <h2>Cleanup review</h2><p>Compare the whole account. Review first, then tag selected items. Nothing is dismantled.</p>
    ...
    <div className={styles.controls}>
      <label>Focus<select value={settings.focus} onChange={(e) => change({ focus: e.target.value as CleanupSettings["focus"] })}><option value="both">PvE + PvP</option><option value="pve">PvE</option><option value="pvp">PvP</option></select></label>
      <label>Candidates<select value={settings.location} onChange={(e) => change({ location: e.target.value as CleanupSettings["location"] })}><option value="vault">Vault</option><option value="all">All locations</option></select></label>
    ...
    <details><summary>Advanced preferences and protections</summary>
      ...
      <label><input type="checkbox" checked={settings.preferences} onChange={(e) => change({ preferences: e.target.checked })} /> Preference-based suggestions (requires aggressive review)</label>
      <div className={styles.controls}>{Object.entries(settings.priorities).map(([stat, weight]) => <label key={stat}>{stat}<select value={weight} onChange={(e) => change({ priorities: { ...settings.priorities, [stat]: Number(e.target.value) } })}><option value={0}>Ignore in preference comparisons</option><option value={1}>Protect this stat</option></select></label>)}</div>
      <p>Ignoring Health is optional, including for PvE. Exact and strict comparisons still consider every stat.</p>
      <p>Sources supplement owned-inventory comparisons only when preference suggestions and aggressive review are enabled. All selected catalogs must agree; unknown ratings are not negative evidence.</p>
      ...
      {(["weaponShader", "armorShader"] as const).map((key) => <label key={key}>{key === "weaponShader" ? "Weapon shader" : "Armor shader"}<select value={settings.cosmetics?.[key] || ""} onChange={(e) => change({ cosmetics: { enabled: false, ornaments: {}, ...settings.cosmetics, [key]: e.target.value || undefined } })}><option value="">Leave unchanged</option>{[...new Map((analysis?.cosmetics || []).filter((c) => c.kind === "shader" && (key === "weaponShader" ? c.group === "weapons" : c.group !== "weapons")).map((c) => [c.hash, c])).values()].map((c) => <option key={c.hash} value={c.hash}>{c.name}</option>)}</select></label>)}
      {[...new Set((analysis?.cosmetics || []).filter((c) => c.kind === "ornament").map((c) => c.group))].map((group) => <label key={group}>{group} ornament<select value={settings.cosmetics?.ornaments[group] || ""} onChange={(e) => change({ cosmetics: { enabled: false, ...settings.cosmetics, ornaments: { ...settings.cosmetics?.ornaments, [group]: e.target.value } } })}><option value="">Leave unchanged</option>{analysis?.cosmetics.filter((c) => c.kind === "ornament" && c.group === group).map((c) => <option key={c.hash} value={c.hash}>{c.name}</option>)}</select></label>)}
    ...
    {analysis && <>
      ...
      <div className={styles.controls}>
        <label>Confidence<select value={confidence} onChange={(e) => setConfidence(Number(e.target.value))}><option value={0}>All recommendations</option><option value={95}>95% and above</option><option value={99}>99% exact comparisons</option></select></label>
