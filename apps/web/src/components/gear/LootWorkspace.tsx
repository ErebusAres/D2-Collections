...
export function LootWorkspace({ timeline, loading = false, error, warnings = [], onRetry, onTag, onPull, onSocketChange, busy }: LootWorkspaceProps) {
  ...
  return <>
    ...
    <RecentEventRow title="Recent Weapons" subtitle="Physical weapon instances · newest to oldest" events={weapons} onTag={onTag} onPull={onPull} onSocketChange={onSocketChange} busy={busy} empty={`No recent weapons${emptySuffix}`} />
    <RecentEventRow title="Recent Armor" subtitle="Physical armor instances · newest to oldest" events={armor} onTag={onTag} onPull={onPull} onSocketChange={onSocketChange} busy={busy} empty={`No recent armor items${emptySuffix}`} />
