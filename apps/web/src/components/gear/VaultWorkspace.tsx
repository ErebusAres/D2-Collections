...
export function VaultWorkspace({ data, selectedCharacterId, onTag, onAction, busy }: {
  ...
  return <>
    ...
    <LootHistoryGrid title="Vault contents" subtitle={visible.length < filtered.length ? `Showing ${visible.length} of ${filtered.length} matching items · hover or focus for full details` : "Only physical weapons and armor currently reported in your Vault · hover or focus for full details"} items={visible} onTag={onTag} onSocketChange={(item, socketIndex, plugItemHash) => onAction({ action: "setWeaponSocket", itemInstanceId: item.instanceId, characterId: selectedCharacterId, socketIndex, plugItemHash })} busy={busy} empty="No Vault items match these filters. Reset the filters or sync gear after changing your inventory in Destiny." itemActions={(item) => <VaultItemActions item={item} selectedCharacterId={selectedCharacterId} onAction={onAction} busy={busy} />} />
