...
export function CleanupSelect({ value, onChange, children }: { value: string | number; onChange: (event: { target: { value: string } }) => void; children: ReactNode }) {
  ...
  return <span ref={root} className={styles.picker} onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); setOpen(false); trigger.current?.focus(); } }}>
    <button type="button" ref={trigger} className={styles.trigger} aria-expanded={open} aria-controls={id} onClick={() => { setOpen(!open); setQuery(""); }}>{selected?.label || "Choose…"}<ChevronDown size={14} /></button>
    {open && <span id={id} className={styles.menu} role="group" aria-label="Available choices">
      ...
      <span className={styles.options}>{options.filter((option) => option.label.toLowerCase().includes(query.toLowerCase())).map((option) => <button type="button" className={styles.option} key={option.value} aria-pressed={option.value === String(value)} onClick={() => { onChange({ target: { value: option.value } }); setOpen(false); trigger.current?.focus(); }}><span>{option.label}</span>{option.value === String(value) && <Check size={14} />}</button>)}</span>
