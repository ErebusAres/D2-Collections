...
import { classIconFor } from "./BuildIcon";
...
export function BuildIdentitySelector({ value, onChange }: { value: BuildDocument; onChange: (value: BuildDocument) => void }) {
  ...
  const classDefinition = classCatalog.data?.data.results.find((entry) => entry.classType === value.classType) || { icon: classIconFor(value.classType) };
  const resolvedClassIcon = classDefinition?.icon || value.classIcon || classIconFor(value.classType);
  ...
  useEffect(() => {
    ...
    const classIcon = resolvedClassIcon;
  ...
  }, [definition?.icon, onChange, resolvedClassIcon, transcendence, value]);
  ...
  return <div className={styles.identitySelectors}>
    ...
    <div className={styles.identityManifestPreview}>
      <img src={definition?.icon || resolvedClassIcon} alt="" />
