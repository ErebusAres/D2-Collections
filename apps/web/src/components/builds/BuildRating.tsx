...
export function BuildRating({ buildId, rating, viewerVote, compact = false, disabled = false, onChange }: { buildId: string; rating: BuildRatingData; viewerVote?: BuildVoteValue; compact?: boolean; disabled?: boolean; onChange?: (result: BuildVoteResult) => void }) {
  ...
  const cast = (value: BuildVoteValue) => {
    if (disabled) return;
    if (session?.authenticated) vote.mutate(value); else signIn();
  };
  return <div className={`${styles.rating} ${compact ? styles.compactRating : ""}`} data-tone={tone} aria-label={`${rating.upvotes} upvotes, ${rating.downvotes} downvotes`}>
    <button type="button" className={viewerVote === "up" ? styles.selectedVote : ""} disabled={vote.isPending || disabled} onClick={() => cast("up")} title={disabled ? "Publish this draft before voting." : session?.authenticated ? "Upvote this build" : "Sign in to vote"}><ThumbsUp /><span>{rating.upvotes}</span></button>
    <strong>{rating.total ? `${rating.percentPositive}%` : "New"}</strong>
    <button type="button" className={viewerVote === "down" ? styles.selectedVote : ""} disabled={vote.isPending || disabled} onClick={() => cast("down")} title={disabled ? "Publish this draft before voting." : session?.authenticated ? "Downvote this build" : "Sign in to vote"}><ThumbsDown /><span>{rating.downvotes}</span></button>
  </div>;
}
