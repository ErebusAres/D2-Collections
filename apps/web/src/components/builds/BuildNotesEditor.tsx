...
export function BuildNotesEditor({ value, onChange }: { value: BuildDocument; onChange: (value: BuildDocument) => void }) {
  ...
  return <div className={styles.notesInsertEditor}>
    ...
    <ManifestPicker kind="icon" label="Insert an official Destiny icon at the cursor" placeholder="Search elements, champions, classes, abilities, gear, perks, mods, Artifacts, or engrams…" context={{ classType: value.classType, subclass: value.subclass }} allowManual={false} onSelect={(entry) => insert(entry)} />
  </div>;
}
