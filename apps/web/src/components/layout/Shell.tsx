...
import { ArrowUp, Badge, Boxes, Coins, Crosshair, Database, Gift, GitCompareArrows, Hammer, Layers3, ListTodo, Mail, Orbit, ScanSearch, Settings, ShieldEllipsis, Sparkles, Ticket, Users, Wrench } from "lucide-react";
          ...
          <div className={styles.guardianIdentity}>
            {guardian ? (
              <>
                ...
                <div className={styles.identityDetails}><span>Selected Guardian</span><strong>{guardian.displayName}</strong><small>{character?.className} · {character?.raceName}</small>{character?.emblemBackgroundPath && <div className={styles.guardianBanner} data-testid="guardian-banner" aria-hidden="true" />}<div className={styles.identityStats} aria-label="Guardian stats"><HeaderStat label="Light Level" value={guardian.stats.power} icon={<Sparkles />} accent to="/power" /><HeaderStat label="Guardian Rank" value={guardian.stats.guardianRank} icon={<Badge />} to="/guardian-rank" /><HeaderStat label="Crucible Rank" value={guardian.stats.crucibleRank?.level} detail={guardian.stats.crucibleRank?.rankName} icon={<Crosshair />} to="/pvp" /><HeaderStat label="Rewards Pass" value={rewardsPassProgress?.state === "unavailable" && !rewardsPassRank ? undefined : rewardsPassRank} icon={<Ticket />} to="/rewards" claimable={claimableReward} /><HeaderStat label="Mailbox" value={guardian.stats.mailboxCount} icon={<Mail />} to="/mailbox" /><HeaderStat label="Reward Codes" value={availableRewardCodeCount} icon={<Gift />} to="/codes" claimable={availableRewardCodeCount > 0} /></div>{rewardsPassProgress && <RewardsProgress rank={rewardsPassRank} progress={rewardsPassProgress} />}</div>
