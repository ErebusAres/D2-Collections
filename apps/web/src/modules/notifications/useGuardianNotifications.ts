...
export function useGuardianNotifications(history = false): GuardianNotificationsController {
  ...
  const notifications = useMemo(() => {
    ...
    [...(query.data?.data.notifications || []), ...rewardNotifications].forEach((entry) => {
      ...
      byId.set(entry.id, { ...entry, ...local, status: local?.dismissedAt ? "dismissed" : local?.archivedAt ? "archived" : local?.readAt ? "read" : entry.status });
    });
  ...
  }, [localState, preferences, query.data?.data.notifications, rewardNotifications]);
  const feed = notifications.filter((entry) => !entry.dismissedAt && entry.status === "active")
