...
import { ArchiveRestore, CalendarClock, Check, Circle, ExternalLink, FolderKanban, IdCard, Plus, SkipForward, Trash2, Users } from "lucide-react";
  ...
  return <AuthGate>
    ...
    <section className={styles.layout}>
      <form className={styles.creator} onSubmit={submit}>
        ...
        <p>{document.projects.length}/20 projects · up to 24 checklist items each.</p>
      </form>
      <section className={styles.board}>
        ...
        <div className={styles.cards}>{projects.map((project) => {
          ...
          return <article key={project.id}>
            ...
            <footer>{project.sourceUrl && <a href={project.sourceUrl} target="_blank" rel="noreferrer">Reference <ExternalLink /></a>}<button type="button" onClick={() => patchProject(project.id, { completedAt: project.completedAt ? undefined : new Date().toISOString() })}><ArchiveRestore /> {project.completedAt ? "Restore" : "Complete"}</button><button type="button" className={styles.delete} aria-label={`Delete ${project.title}`} onClick={() => remove(project.id)}><Trash2 /></button></footer>
          </article>;
        })}</div>
      </section>
    </section>
