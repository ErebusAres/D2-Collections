...
function QuestReport({ quest }: { quest: QuestProgress }) {
  ...
  return <>
    <section className={styles.questDetailHero}><div>{quest.icon ? <img src={quest.icon} alt="" /> : <Crosshair />}</div><main><span>{quest.activityName || "Active pursuit"}</span><h2>{quest.name}</h2><p>{quest.description}</p><div><b>{quest.percent}% current-step progress</b>{quest.stepNumber && quest.stepCount && <b>Step {quest.stepNumber} of {quest.stepCount}</b>}{quest.inGameTracked && <b>Tracked in Destiny</b>}</div></main></section>
    <section className={styles.questDetailGrid}>
      ...
      <aside className={styles.questReportAside}>
        <section><header><Gift /><h2>Rewards</h2></header>{quest.rewards.length ? <ul>{quest.rewards.map((reward) => <li key={reward}>{reward}</li>)}</ul> : <p>Bungie does not list a reward on this active step.</p>}</section>
