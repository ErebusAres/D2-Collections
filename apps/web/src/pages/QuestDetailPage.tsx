...
function StepGuide({ guide }: { guide: NonNullable<QuestStepProgress["guide"]> }) {
  return <details className={styles.questStepGuide} open={guide.coverage === "curated"}>
    <summary><Lightbulb />Step guide <span>{guide.coverage === "curated" ? "Verified walkthrough" : "Objective-specific"}</span></summary>
    <p>{guide.summary}</p>
    {guide.steps.length > 0 && <ol>{guide.steps.map((entry, index) => <li key={index}>{entry}</li>)}</ol>}
    {guide.tips.length > 0 && <section><strong>Efficiency tips</strong><ul>{guide.tips.map((entry, index) => <li key={index}>{entry}</li>)}</ul></section>}
    {guide.warnings.length > 0 && <section><strong>Watch for</strong><ul>{guide.warnings.map((entry, index) => <li key={index}>{entry}</li>)}</ul></section>}
    {guide.secrets.length > 0 && <section><strong>Secrets</strong><ul>{guide.secrets.map((entry, index) => <li key={index}>{entry}</li>)}</ul></section>}
  </details>;
}
