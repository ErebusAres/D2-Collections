  ...
  return <AuthGate>
    ...
    {data && <>
      ...
      <section className={styles.panel}>
        ...
        <div className={styles.tableWrap}><table><thead><tr><th>Guardian</th><th>Membership ID</th><th>Class</th><th>Power</th><th>Rank / Pass</th><th>First login</th><th>Last sign-in</th><th title="Most recent browser session check; recorded at most once every five minutes. Background jobs do not count.">Last seen</th><th>Sessions</th><th>Admin action</th></tr></thead><tbody>
          {data.logins.map((row) => {
      ...
      <section className={styles.panel}>
        ...
        <div className={styles.tableWrap}><table><thead><tr><th>Visitor</th><th>First seen</th><th>Approximate country / region</th><th>Preferred language</th><th>Sampled</th></tr></thead><tbody>{data.visitors.map((row) => <tr key={row.visitorId}><td><code>{row.visitorId}</code></td><td>{dateTime(row.firstSeenAt)}</td><td>{[row.region, countryName(row.country)].filter(Boolean).join(", ") || (row.locationSampledAt ? "Unavailable" : "Not previously recorded")}</td><td>{row.preferredLanguage || (row.locationSampledAt ? "Unavailable" : "Not previously recorded")}</td><td>{row.locationSampledAt ? dateTime(row.locationSampledAt) : "Awaiting return visit"}</td></tr>)}</tbody></table></div>
      </section>
    </>}
  </AuthGate>;
}
