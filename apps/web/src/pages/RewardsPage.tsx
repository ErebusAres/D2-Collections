  ...
  return <AuthGate>
    ...
    {data && <>
      <section className={styles.rewardsHero} style={data.backgroundImage ? { "--rewards-background": `url(${data.backgroundImage})` } as React.CSSProperties : undefined}>
        ...
        <div className={`${styles.rewardsProgress} ${levelProgress ? "" : styles.rewardsProgressUnavailable}`}><header><div><span>{levelProgress?.mode === "bright-engram" ? "Bright Engram progress" : levelProgress ? "Next rank" : "XP status"}</span><strong>{levelProgress?.mode === "bright-engram" ? "Five-level cycle" : levelProgress ? data.rank + 1 : "Unavailable"}</strong></div><b>{levelProgress ? <><strong>{levelProgress.percent}%</strong><small>{levelProgress.mode === "bright-engram" ? "of engram" : "of level"}</small></> : "—"}</b></header>{levelProgress?.segments ? <div className={styles.rewardsProgressSegments}>{levelProgress.segments.map((percent, index) => <i key={index}><span style={{ width: `${percent}%` }} /></i>)}</div> : <i><span style={{ width: `${levelProgress?.percent || 0}%` }} /></i>}<p>{levelProgress ? levelProgress.mode === "bright-engram" ? `${levelProgress.current.toLocaleString()} / ${levelProgress.required.toLocaleString()} XP toward the next Bright Engram · current segment ${levelProgress.levelCurrent.toLocaleString()} / ${levelProgress.levelRequired.toLocaleString()} XP` : `${levelProgress.current.toLocaleString()} / ${levelProgress.required.toLocaleString()} XP this level${progress?.currentProgress !== undefined ? ` · ${progress.currentProgress.toLocaleString()} total progression` : ""}` : progress?.reason || "Bungie did not return a usable XP threshold."}</p></div>
      ...
      <section className={styles.rewardInsights} aria-label="Rewards Pass status">
        <article><Gauge /><div><span>Live progression</span><strong>{levelProgress ? levelProgress.mode === "bright-engram" ? `Rank ${data.rank} · Bright Engram ${levelProgress.percent}%` : `Rank ${data.rank} · ${levelProgress.percent}% complete` : `Rank ${data.rank}`}</strong><p>{levelProgress ? `${(levelProgress.required - levelProgress.current).toLocaleString()} XP until ${levelProgress.mode === "bright-engram" ? "the next Bright Engram" : `rank ${data.rank + 1}`}` : progress?.reason || "Next-rank XP is unavailable from Bungie."}</p></div></article>
