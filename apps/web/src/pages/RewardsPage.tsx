  ...
  return <AuthGate>
    ...
    {data && <>
      <section className={styles.rewardsHero} style={data.backgroundImage ? { "--rewards-background": `url(${data.backgroundImage})` } as React.CSSProperties : undefined}>
        ...
        <div className={`${styles.rewardsProgress} ${levelProgress ? "" : styles.rewardsProgressUnavailable}`}><header><div><span>{levelProgress ? "Next rank" : "XP status"}</span><strong>{levelProgress ? data.rank + 1 : "Unavailable"}</strong></div><b>{levelProgress ? <><strong>{levelProgress.percent}%</strong><small>{levelProgress.segments ? "of rank" : "of level"}</small></> : "—"}</b></header>{levelProgress?.segments ? <div className={styles.rewardsProgressSegments}>{levelProgress.segments.map((percent, index) => <i key={index}><span style={{ width: `${percent}%` }} /></i>)}</div> : <i><span style={{ width: `${levelProgress?.percent || 0}%` }} /></i>}<p>{levelProgress ? levelProgress.segments ? `${levelProgress.current.toLocaleString()} / ${levelProgress.required.toLocaleString()} XP toward rank ${data.rank + 1} · post-100 ranks fill five pips one at a time` : `${levelProgress.current.toLocaleString()} / ${levelProgress.required.toLocaleString()} XP this level${progress?.currentProgress !== undefined ? ` · ${progress.currentProgress.toLocaleString()} total progression` : ""}` : progress?.reason || "Bungie did not return a usable XP threshold."}</p></div>
      ...
      <section className={styles.rewardInsights} aria-label="Rewards Pass status">
        <article><Gauge /><div><span>Live progression</span><strong>{levelProgress ? `Rank ${data.rank} · ${levelProgress.percent}% to ${data.rank + 1}` : `Rank ${data.rank}`}</strong><p>{levelProgress ? `${(levelProgress.required - levelProgress.current).toLocaleString()} XP until rank ${data.rank + 1}` : progress?.reason || "Next-rank XP is unavailable from Bungie."}</p></div></article>
