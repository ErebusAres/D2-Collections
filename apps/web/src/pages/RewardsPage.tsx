...
export function RewardsPage() {
  ...
  const postRankSegments = levelProgress?.segments;
  const completedPostRankSegments = levelProgress?.completedSegments ?? postRankSegments?.filter((percent) => percent >= 100).length ?? 0;
  const totalPostRankSegments = levelProgress?.totalSegments ?? postRankSegments?.length ?? 0;
  ...
  return <AuthGate>
    ...
    {data && <>
      <section className={styles.rewardsHero} style={data.backgroundImage ? { "--rewards-background": `url(${data.backgroundImage})` } as React.CSSProperties : undefined}>
        ...
        <div className={`${styles.rewardsProgress} ${levelProgress ? "" : styles.rewardsProgressUnavailable}`}><header><div><span>{postRankSegments ? "Rank pip progress" : levelProgress ? "Next rank" : "XP status"}</span><strong>{levelProgress ? postRankSegments ? data.rank : data.rank + 1 : "Unavailable"}</strong></div><b>{levelProgress ? postRankSegments ? <><strong>{completedPostRankSegments}/{totalPostRankSegments}</strong><small>pips</small></> : <><strong>{levelProgress.percent}%</strong><small>of level</small></> : "—"}</b></header>{postRankSegments ? <div className={styles.rewardsProgressSegments}>{postRankSegments.map((percent, index) => <i key={index}><span style={{ width: `${percent}%` }} /></i>)}</div> : <i><span style={{ width: `${levelProgress?.percent || 0}%` }} /></i>}<p>{levelProgress ? postRankSegments ? `${levelProgress.current.toLocaleString()} / ${levelProgress.required.toLocaleString()} XP in rank ${data.rank} · ${completedPostRankSegments}/${totalPostRankSegments} pips toward rank ${data.rank + 1}` : `${levelProgress.current.toLocaleString()} / ${levelProgress.required.toLocaleString()} XP this level${progress?.currentProgress !== undefined ? ` · ${progress.currentProgress.toLocaleString()} total progression` : ""}` : progress?.reason || "Bungie did not return a usable XP threshold."}</p></div>
      ...
      <section className={styles.rewardCatalog}>
        ...
        {data.rewardDataState === "available" ? <>
          ...
          <div className={styles.rewardTrackWindow}><div className={styles.rewardLevelGrid}>{visibleLevels.map((entry) => <RewardLevelColumn key={entry.level} entry={entry} currentRank={data.rank} currentRankSegments={postRankSegments} />)}</div></div>
      ...
      <section className={styles.rewardInsights} aria-label="Rewards Pass status">
        <article><Gauge /><div><span>Live progression</span><strong>{levelProgress ? postRankSegments ? `Rank ${data.rank} · ${completedPostRankSegments}/${totalPostRankSegments} pips` : `Rank ${data.rank} · ${levelProgress.percent}% to ${data.rank + 1}` : `Rank ${data.rank}`}</strong><p>{levelProgress ? `${Math.max(0, levelProgress.required - levelProgress.current).toLocaleString()} XP until rank ${data.rank + 1}` : progress?.reason || "Next-rank XP is unavailable from Bungie."}</p></div></article>
