...
import { CompactRecentLootBar, recentLoot, type LootItem } from "../components/gear/RecentLoot";
...
export function FireteamPage() {
  ...
  return <AuthGate>
    ...
    {data && <>
      ...
      {showRecentLoot ? <CompactRecentLootBar items={recentLoot(recentGear.data?.data.items || [], recentGear.data?.data.weapons || [], "all", 5)} onTag={tagRecent} busy={gearState.isPending} onHide={() => setPreference("fireteam.recentLoot.v1", "off")} trailing={observedCatalysts.slice(0, 2).map((catalyst) => <Link className={styles.fireteamCatalystChip} key={catalyst.recordHash} to="/collection?view=catalysts" title={`${catalyst.name} · ${catalyst.state === "complete" ? "Completed" : `${catalyst.percent}% obtained`}`}>{catalyst.icon && <img src={catalyst.icon} alt="" />}<span><b>{catalyst.name}</b><small>Catalyst</small></span></Link>)} /> : <section className={styles.fireteamLootControl}><div><strong>Recent account loot hidden</strong><small>Private to you</small></div><button onClick={() => setPreference("fireteam.recentLoot.v1", "on")}>Show bar</button></section>}
