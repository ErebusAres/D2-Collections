...
import { BookOpen, Check, ChevronRight, Search, Shield, Sparkles, Swords, X } from "lucide-react";
...
function FilterGroup({ label, value, values, onChange }: { label: string; value: string; values: string[]; onChange: (value: string) => void }) {
  return <div className={styles.filterGroup}><span>{label}</span><div>{values.map((entry) => <button key={entry} className={value === entry ? styles.activeFilter : ""} onClick={() => onChange(entry)}>{entry}</button>)}</div></div>;
}
...
function StateBadge({ active, label, gold }: { active: boolean; label: string; gold?: boolean }) {
  return <span className={`${styles.stateBadge} ${active ? styles.stateActive : ""} ${gold ? styles.stateGold : ""}`}><i>{active && <Check size={10} />}</i>{label}</span>;
}
