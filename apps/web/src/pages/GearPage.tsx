...
export function GearPage() {
  ...
  const workspace = preferences["gear.workspace"] === "weapons" ? "weapons" : preferences["gear.workspace"] === "loot" ? "loot" : preferences["gear.workspace"] === "vault" ? "vault" : "armor";
  ...
  return <AuthGate><PageHeader eyebrow="Account-wide gear intelligence" title="Gear" description={workspace === "weapons" ? "Compare physical weapon rolls and selectable perks without opaque god-roll scores or dismantling automation." : workspace === "loot" ? "Browse separate newest-to-oldest histories for weapons, armor, catalysts, engrams, materials, and other inventory gains." : workspace === "vault" ? "Search and filter only the physical weapons and armor currently stored in your Vault." : "Compare true base distributions, audit every adjustment, and manage armor across your characters and vault."} actions={<><Freshness observedAt={result.data?.freshness.observedAt} warning={result.data?.warnings[0]} /><button className={styles.gearRefresh} onClick={refresh}><RefreshCw size={14} /> Sync gear</button></>} />
    <QueryState loading={result.isLoading} error={result.error as Error} hasData={Boolean(data)} onRetry={() => void result.refetch()} />
    {data && <>
      <nav className={styles.gearWorkspaceTabs} aria-label="Gear workspace"><button data-active={workspace === "armor"} onClick={() => setPreference("gear.workspace", "armor")}>Armor <b>{data.totals.armor}</b></button><button data-active={workspace === "weapons"} onClick={() => setPreference("gear.workspace", "weapons")}>Weapons <b>{data.weapons?.length || 0}</b></button><button data-active={workspace === "loot"} onClick={() => setPreference("gear.workspace", "loot")}>Loot <b>{recentItems?.events.length || 0}</b></button><button data-active={workspace === "vault"} onClick={() => setPreference("gear.workspace", "vault")}>Vault <b>{data.items.filter((item) => item.location === "vault").length + (data.weapons || []).filter((item) => item.location === "vault").length}</b></button></nav>
