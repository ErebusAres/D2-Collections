import type { ArmorItem, ArmorStatKey, GearActionRequest, GearActionResult, GearData, GearTag, RecentItemTimelineData } from "@guardian-nexus/contracts";
...
import { CleanupWorkspace } from "../components/gear/CleanupWorkspace";
import { VaultWorkspace } from "../components/gear/VaultWorkspace";
import { RecentItemRow, recentLoot, type LootItem } from "../components/gear/RecentLoot";
import { ItemAppearance } from "../components/gear/ItemAppearance";
import { GearTierRail } from "../components/gear/GearTierRail";
...
export function GearPage() {
  ...
  const workspace = preferences["gear.workspace"] === "cleanup" ? "cleanup" : preferences["gear.workspace"] === "weapons" ? "weapons" : preferences["gear.workspace"] === "loot" ? "loot" : preferences["gear.workspace"] === "vault" ? "vault" : "armor";
  ...
  const recentItemsResult = useQuery({ queryKey: ["recent-items", selectedCharacterId], queryFn: () => api<RecentItemTimelineData>(`/api/v1/me/recent-items?characterId=${encodeURIComponent(selectedCharacterId)}`), enabled: Boolean(session?.authenticated && selectedCharacterId), staleTime: 30_000, refetchInterval: autoRefresh ? 60_000 : false, refetchIntervalInBackground: false });
  const data = result.data?.data;
  const recentItems = recentItemsResult.data?.data;
  ...
  const items = useMemo(() => {
    const values = (data?.items || []).filter((item) => (allClasses || item.className === data?.selectedClass || item.className === "Unknown") && (slot === "all" || item.slot === slot) && (location === "all" || item.location === location) && (tag === "all" || (tag === "cleanup" ? Boolean(item.cleanupRecommendation) : tag === "none" ? !item.tag : item.tag === tag)) && (!onlyGrouped || groupedIds.has(item.instanceId)) && matchesGearSearch(item, search, { groupId: groupedIds.get(item.instanceId)?.label }));
  ...
  const refresh = () => { void queryClient.invalidateQueries({ queryKey: ["gear", selectedCharacterId] }); void queryClient.invalidateQueries({ queryKey: ["recent-items", selectedCharacterId] }); };
  const stateMutation = useMutation({ mutationFn: (input: { itemInstanceId: string; tag?: GearTag | null; dismissed?: boolean }) => queuedApi("/api/v1/me/gear/item-state", { method: "PUT", headers: mutationHeaders(session?.csrfToken), body: JSON.stringify(input) }, { persist: true }), onMutate: async (input) => { const keys = [["gear", selectedCharacterId], ["recent-items", selectedCharacterId]] as const; await Promise.all(keys.map((queryKey) => queryClient.cancelQueries({ queryKey }))); const previous = keys.map((queryKey) => queryClient.getQueryData(queryKey)); keys.forEach((queryKey) => queryClient.setQueryData(queryKey, (value: unknown) => updateCachedGearTag(value, input.itemInstanceId, input.tag || undefined))); return { keys, previous }; }, onError: (_error, _input, context) => context?.keys.forEach((queryKey, index) => queryClient.setQueryData(queryKey, context.previous[index])), onSettled: refresh });
  const actionMutation = useMutation({ mutationFn: async (input: GearActionRequest) => { const response = await api<GearActionResult>("/api/v1/me/gear/action", { method: "POST", headers: mutationHeaders(session?.csrfToken), body: JSON.stringify(input) }); if (response.data.failed[0]) throw new Error(response.data.failed[0].message); return response; }, onSuccess: refresh });
  ...
  return <AuthGate><PageHeader eyebrow="Your account-wide gear" title="Gear" description={workspace === "cleanup" ? "Review duplicate gear, keep useful alternatives, and mark approved items without changing your manual tags." : workspace === "weapons" ? "Compare every weapon roll, view all available traits, and see separate PvE and PvP ratings." : workspace === "loot" ? "Review your newest weapons, armor, catalysts, engrams, materials, and other items." : workspace === "vault" ? "Search and filter the weapons and armor currently stored in your Vault." : "Compare armor stats, see every modifier, and manage pieces across your characters and Vault."} actions={<><button className={styles.gearRefresh} onClick={() => { setCleanupRequest((value) => value + 1); setPreference("gear.workspace", "cleanup"); }}>Analyze cleanup</button><Freshness observedAt={result.data?.freshness.observedAt} warning={result.data?.warnings[0]} /><button className={styles.gearRefresh} onClick={refresh}><RefreshCw size={14} /> Sync gear</button></>} />
    <QueryState loading={result.isLoading} error={result.error as Error} hasData={Boolean(data)} onRetry={() => void result.refetch()} />
    {data && <>
      <nav className={styles.gearWorkspaceTabs} aria-label="Gear workspace"><button data-active={workspace === "armor"} onClick={() => setPreference("gear.workspace", "armor")}>Armor <b>{data.totals.armor}</b></button><button data-active={workspace === "weapons"} onClick={() => setPreference("gear.workspace", "weapons")}>Weapons <b>{data.weapons?.length || 0}</b></button><button data-active={workspace === "loot"} onClick={() => setPreference("gear.workspace", "loot")}>Loot <b>{recentItems?.events.length || 0}</b></button><button data-active={workspace === "vault"} onClick={() => setPreference("gear.workspace", "vault")}>Vault <b>{data.items.filter((item) => item.location === "vault").length + (data.weapons || []).filter((item) => item.location === "vault").length}</b></button><button data-active={workspace === "cleanup"} onClick={() => setPreference("gear.workspace", "cleanup")}>Cleanup</button></nav>
      {workspace === "cleanup" ? <CleanupWorkspace onTag={tagLoot} analysisRequest={cleanupRequest} /> : workspace === "vault" ? <VaultWorkspace data={data} selectedCharacterId={selectedCharacterId} onTag={tagLoot} onAction={(input, confirm) => (!confirm || window.confirm(confirm)) && actionMutation.mutate(input)} busy={stateMutation.isPending || actionMutation.isPending} /> : workspace === "loot" ? <LootWorkspace timeline={recentItems} loading={recentItemsResult.isLoading} error={recentItemsResult.error as Error | null} warnings={recentItemsResult.data?.warnings} onRetry={() => void recentItemsResult.refetch()} onTag={tagLoot} onPull={(item) => actionMutation.mutate({ action: "transfer", itemInstanceId: item.instanceId, target: "character", targetCharacterId: selectedCharacterId })} onSocketChange={(item, socketIndex, plugItemHash) => actionMutation.mutate({ action: "setWeaponSocket", itemInstanceId: item.instanceId, characterId: selectedCharacterId, socketIndex, plugItemHash })} busy={stateMutation.isPending || actionMutation.isPending} /> : workspace === "weapons" ? <WeaponWorkspace data={data} selectedCharacterId={selectedCharacterId} preferences={preferences} setPreference={setPreference} onTag={(item, value) => stateMutation.mutate({ itemInstanceId: item.instanceId, tag: value || null })} onAction={(input, confirm) => (!confirm || window.confirm(confirm)) && actionMutation.mutate(input)} busy={stateMutation.isPending || actionMutation.isPending} /> : <>
...
function ArmorCard({ item, statIcons, group, selectedCharacterId, onCompare, onTag, onAction, busy }: { item: ArmorItem; statIcons: GearData["statIcons"]; group?: ReturnType<typeof groupArmor>[number]; selectedCharacterId: string; onCompare: (id: string) => void; onTag: (tag: "" | GearTag) => void; onAction: (input: GearActionRequest, confirm?: string) => void; busy: boolean }) {
  ...
  return <article tabIndex={0} data-gear-instance={item.instanceId} onMouseEnter={activateGearShortcut} onMouseLeave={deactivateGearShortcut} onFocus={activateGearShortcut} onBlur={deactivateGearShortcut} className={`${styles.armorCard} ${item.rarity === "Exotic" ? styles.exoticArmor : ""} ${item.masterworked ? styles.masterworkedArmor : ""} ${groupClass}`}>
    ...
    <ItemAppearance itemId={item.instanceId} />
    <footer><GearTagPicker value={item.tag} onChange={(value) => onTag(value || "")} disabled={busy} compact /><span className={styles.footerSpacer} /><button title={item.locked ? "Unlock" : "Lock"} aria-label={`${item.locked ? "Unlock" : "Lock"} ${item.name}`} onClick={() => onAction({ action: "setLock", itemInstanceId: item.instanceId, locked: !item.locked, characterId: item.ownerCharacterId || selectedCharacterId })}>{item.locked ? <Lock size={13} /> : <LockOpen size={13} />}</button>{item.location === "vault" ? <button title="Pull to Selected Guardian" aria-label={`Pull ${item.name} to Selected Guardian`} onClick={() => onAction({ action: "transfer", itemInstanceId: item.instanceId, target: "character", targetCharacterId: selectedCharacterId })}><ArrowDownToLine size={13} /></button> : <button disabled={item.equipped} title={item.equipped ? "Equip another item before vaulting this armor" : "Move to vault"} aria-label={item.equipped ? `${item.name} is equipped and cannot be moved to the vault` : `Move ${item.name} to vault`} onClick={() => onAction({ action: "transfer", itemInstanceId: item.instanceId, target: "vault" })}><ArrowUpFromLine size={13} /></button>}<button title="Equip on Selected Guardian" aria-label={`Equip ${item.name} on Selected Guardian`} onClick={() => onAction({ action: "equip", itemInstanceId: item.instanceId, characterId: selectedCharacterId }, `Equip ${item.name} on the Selected Guardian? This may move it between characters first.`)}><Shield size={13} /></button></footer>
  </article>;
}
...
function updateCachedGearTag(value: unknown, itemInstanceId: string, tag?: GearTag): unknown {
  if (!value || typeof value !== "object") return value;
  const root = value as any;
  const update = (item: any) => item?.instanceId === itemInstanceId ? { ...item, tag } : item;
  const data = root.data;
  if (!data) return value;
  return { ...root, data: { ...data, ...(Array.isArray(data.items) ? { items: data.items.map(update) } : {}), ...(Array.isArray(data.weapons) ? { weapons: data.weapons.map(update) } : {}), ...(Array.isArray(data.events) ? { events: data.events.map((event: any) => event.gear?.instanceId === itemInstanceId ? { ...event, gear: update(event.gear) } : event) } : {}) } };
}
...
function ComparisonStation({ group, statIcons, selectedCharacterId, onClose, onAction }: { group: ReturnType<typeof groupArmor>[number]; statIcons: GearData["statIcons"]; selectedCharacterId: string; onClose: () => void; onAction: (input: GearActionRequest, message: string) => void }) {
  ...
  return <div className={styles.compareOverlay}><button aria-label="Close comparison" onClick={onClose} /><section><header><div><span>Armor comparison station</span><h2>{group.label}</h2><p>{group.items.length} similar items · base-stat order</p></div><div>{vaultIds.length > 0 && <button onClick={() => onAction({ action: "groupPull", itemInstanceIds: vaultIds, characterId: selectedCharacterId }, `Pull ${vaultIds.length} armor items from the vault?`)}><ArrowDownToLine size={14} /> Pull group ({vaultIds.length})</button>}<button onClick={onClose} aria-label="Close armor comparison"><X size={18} /></button></div></header><div className={styles.compareGrid}>{group.items.map((item, index) => <div key={item.instanceId} className={styles.compareArmor}><span>{index === 0 ? "Best base total" : item.location}</span><h3>{item.name}</h3>{item.icon && <img className={styles.compareItemIcon} src={item.icon} alt="" />}<strong>{item.baseTotal} base / {item.currentTotal} current</strong>{ARMOR_STAT_KEYS.map((key) => <StatRow key={key} statKey={key} item={item} icon={statIcons[key]} />)}<small>{item.archetype?.icon && <img src={item.archetype.icon} alt="" />}{item.archetype?.name || "Archetype unavailable"}</small></div>)}</div></section></div>;
}
import { CleanupBadge } from "../components/gear/CleanupBadge";
