...
function ArmorCard({ item, statIcons, group, selectedCharacterId, onCompare, onTag, onAction, busy }: { item: ArmorItem; statIcons: GearData["statIcons"]; group?: ReturnType<typeof groupArmor>[number]; selectedCharacterId: string; onCompare: (id: string) => void; onTag: (tag: "" | GearTag) => void; onAction: (input: GearActionRequest, confirm?: string) => void; busy: boolean }) {
  return <article className={`${styles.armorCard} ${item.rarity === "Exotic" ? styles.exoticArmor : ""}`}>
    <header><div className={styles.armorArt}>{item.icon && <img src={item.icon} alt="" />}<b>{item.power || ""}</b></div><div><span>{item.rarity} · {item.slot}</span><h2>{item.name}</h2><p>{item.location}{item.equipped ? " · Equipped" : ""}</p></div><div className={styles.grade}><strong>{item.grade.letter}</strong><small>{item.grade.score ?? "—"}</small></div></header>
    <div className={styles.armorIdentity}>{item.archetype && <span title={item.archetype.description}>{item.archetype.icon && <img src={item.archetype.icon} alt="" />}<b>{item.archetype.name}</b></span>}{item.tuning && <span title={item.tuning.description}>{item.tuning.icon && <img src={item.tuning.icon} alt="" />}<b>{item.tuning.name}</b></span>}{item.setBonuses.slice(0,2).map((bonus) => <span key={bonus.hash} title={bonus.description}>{bonus.icon && <img src={bonus.icon} alt="" />}<b>{bonus.pieces ? `${bonus.pieces}× ` : ""}{bonus.name}</b></span>)}</div>
    <div className={styles.armorStats}>{ARMOR_STAT_KEYS.map((key) => <StatRow key={key} statKey={key} item={item} icon={statIcons[key]} />)}<div className={styles.armorTotal}><span>Base / Current</span><strong>{item.baseTotal} <em>{item.currentTotal}</em></strong></div></div>
    <footer><select value={item.tag || ""} onChange={(event) => onTag(event.target.value as "" | GearTag)} disabled={busy}>{TAGS.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}</select>{group && <button className={styles.groupButton} onClick={() => onCompare(group.id)}>Group {group.label} <ChevronRight size={12} /></button>}<button title={item.locked ? "Unlock" : "Lock"} onClick={() => onAction({ action: "setLock", itemInstanceId: item.instanceId, locked: !item.locked, characterId: item.ownerCharacterId || selectedCharacterId })}>{item.locked ? <Lock size={13} /> : <LockOpen size={13} />}</button>{item.location === "vault" ? <button title="Pull to Selected Guardian" onClick={() => onAction({ action: "transfer", itemInstanceId: item.instanceId, target: "character", targetCharacterId: selectedCharacterId })}><ArrowDownToLine size={13} /></button> : <button disabled={item.equipped} title={item.equipped ? "Equip another item before vaulting this armor" : "Move to vault"} onClick={() => onAction({ action: "transfer", itemInstanceId: item.instanceId, target: "vault" })}><ArrowUpFromLine size={13} /></button>}<button title="Equip on Selected Guardian" onClick={() => onAction({ action: "equip", itemInstanceId: item.instanceId, characterId: selectedCharacterId }, `Equip ${item.name} on the Selected Guardian? This may move it between characters first.`)}><Shield size={13} /></button></footer>
  </article>;
}

function StatRow({ statKey, item, icon }: { statKey: ArmorStatKey; item: ArmorItem; icon?: string }) { const delta = item.currentStats[statKey] - item.baseStats[statKey]; return <div className={styles.armorStat}>{icon ? <img src={icon} alt="" /> : <span /> }<b>{STAT_LABELS[statKey]}</b><i style={{ width: `${Math.min(100, item.baseStats[statKey] / 40 * 100)}%` }} /><strong>{item.baseStats[statKey]}</strong>{delta !== 0 && <em className={delta > 0 ? styles.statPositive : styles.statNegative}>{delta > 0 ? "+" : ""}{delta}</em>}</div>; }
