...
import { useFireteamV2Query } from "../modules/fireteam/useFireteamQuery";
...
const V2_SNAPSHOT_READ_RETRY_MS = 5_000;

export function FireteamV2Route() {
  return <div className={styles.fireteamRoute}>
    <FireteamV2RefreshCountdown />
    <div className={styles.fireteamPageContent}><FireteamPage version="v2" /></div>
  </div>;
}

function FireteamV2RefreshCountdown() {
  const { session, selectedCharacterId } = useGuardian();
  const membershipId = session?.guardian?.membershipId || "";
  const result = useFireteamV2Query(membershipId, selectedCharacterId, Boolean(session?.authenticated));
  ...
  useEffect(() => {
    ...
    const dueMs = data?.refreshState === "delayed" && Number.isFinite(retryMs)
      ? retryMs
      : committedDueMs;
    ...
    const readCommittedSnapshot = async () => {
      ...
      } catch { /* The page keeps the last committed v2 snapshot visible. */ }
      if (!stopped) timer = window.setTimeout(() => void readCommittedSnapshot(), V2_SNAPSHOT_READ_RETRY_MS);
    };
  ...
  }, [data?.pageRefreshDueAt, data?.refreshRetryAt, data?.refreshState, data?.snapshotVersion, result.data, result.isLoading, result.refetch, selectedCharacterId, session?.authenticated]);

  const label = useMemo(() => {
    if (!session?.authenticated || !selectedCharacterId) return "Fireteam v2 refresh unavailable";
    if (Number(data?.snapshotVersion || 0) <= 0) return "Preparing Fireteam v2 snapshot";
    const retryMs = Date.parse(data?.refreshRetryAt || "");
    if (data?.refreshState === "delayed" && Number.isFinite(retryMs) && retryMs > now) {
      ...
      return `Fireteam v2 retry in ${Math.floor(retrySeconds / 60)}:${String(retrySeconds % 60).padStart(2, "0")}`;
    }
    if (refreshing || data?.refreshState === "refreshing" || data?.refreshState === "delayed") return "Refreshing Fireteam v2";
    const dueMs = Date.parse(data?.pageRefreshDueAt || "");
    if (!Number.isFinite(dueMs)) return "Preparing Fireteam v2 snapshot";
    const remainingMs = Math.min(LIVE_REFRESH_INTERVAL_MS, dueMs - now);
    const seconds = Math.max(0, Math.ceil(remainingMs / 1_000));
    return `Fireteam v2 refresh in ${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, "0")}`;
  }, [data?.pageRefreshDueAt, data?.refreshRetryAt, data?.refreshState, data?.snapshotVersion, now, refreshing, selectedCharacterId, session?.authenticated]);
