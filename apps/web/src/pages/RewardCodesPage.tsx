...
export function RewardCodesPage() {
  ...
  return <>
    ...
    <section className={styles.notice}><ShieldCheck /><p>Bungie does not expose universal code-redemption history. Guardian Nexus automatically hides a code only when its <b>exact reward is already acquired in your Destiny Collections</b>. Manual checkmarks sync to your signed-in Guardian Nexus account and retain a browser fallback for offline use. Redemption links pass the code through Bungie’s redemption URL and copy it as a fallback.{warnings.length ? ` ${warnings.join(" ")}` : accountStatus ? " Collection ownership is refreshed periodically and when this tab regains focus." : accountStatusLoading ? " Checking your Collection ownership now." : accountStatusError ? " Collection ownership detection is temporarily unavailable; manual checkmarks still work." : " Sign in to enable account-synced checkmarks and Collection ownership detection."}</p></section>
