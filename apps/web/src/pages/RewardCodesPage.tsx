  ...
  return <>
    ...
    <section className={styles.codeGrid}>{filtered.map((entry) => {
      ...
      return <article key={entry.code} className={`${expired ? styles.expired : ""} ${entry.featured ? styles.featured : ""} ${isHidden ? styles.redeemed : ""}`}>
        ...
        <footer><button onClick={() => void copy(entry.code)}>{copied === entry.code ? <Check /> : <Copy />}{copied === entry.code ? "Copied" : "Copy code"}</button><button className={styles.redeemedAction} aria-pressed={isHidden} disabled={isDetected} title={isDetected ? "This exact reward is already acquired in Destiny Collections." : undefined} onClick={() => setManual(entry.code, !isManual)}><CheckCircle2 />{isDetected ? "Owned" : isManual ? "Unmark" : "Mark used"}</button><a href={rewardCodeRedemptionUrl(entry.code)} target="_blank" rel="noreferrer" onClick={() => void copy(entry.code)}>Copy & redeem <ExternalLink /></a><a href={entry.sourceUrl} target="_blank" rel="noreferrer" title="Open verification source" aria-label={`Open verification source for ${entry.reward}`}><ShieldCheck /></a></footer>
      </article>;
    })}</section>
