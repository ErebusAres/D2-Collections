...
import { cosmeticSocketEntries, cosmeticKind } from "./itemCosmetics";
...
export function cosmeticChoices(profile: any, gear: GearData, manifest: GearManifest): CleanupAnalysis["cosmetics"] {
  ...
  for (const item of [...gear.items, ...(gear.weapons || [])]) {
    ...
    for (const [index] of sockets.entries()) for (const entry of cosmeticSocketEntries(profile, manifest, item.itemHash, item.instanceId, index, item.ownerCharacterId)) {
      if (entry.canInsert !== true || entry.enabled !== true) continue;
      const hash = String(entry.plugItemHash);
      if (result.has(`${group}:${hash}`)) continue;
      const def = manifest.plugDefinitions[hash] as any;
      const kind = cosmeticKind(def);
      if (kind && def?.displayProperties?.name) result.set(`${group}:${hash}`, { hash, name: def.displayProperties.name, icon: imageUrl(def.displayProperties.icon), kind, group });
    }
  }
  return [...result.values()].sort((a, b) => a.name.localeCompare(b.name));
}
function insertable(profile: any, manifest: GearManifest, itemId: string, index: number, hash: string) {
...
export async function markCleanupCosmetics(row: SessionRow, env: Env, itemId: string, characterId: string, settings: CleanupSettings): Promise<string[]> {
  ...
  for (const hash of selected) {
    ...
    const index = sockets.findIndex((_socket, i) => insertable(profile, manifest, itemId, i, hash));
    ...
    if (!insertable(profile, manifest, itemId, index, original)) { warnings.push("Pulled successfully; appearance unchanged because the original cosmetic cannot currently be restored."); continue; }
...
export async function restoreCleanupCosmetics(row: SessionRow, env: Env, itemId: string): Promise<string[]> {
  ...
  for (const change of rows.results || []) {
    ...
    if (current !== change.original_hash) {
      ...
      if (!insertable(profile, manifest, itemId, change.socket_index, change.original_hash)) { warnings.push("Bungie does not currently allow the original cosmetic to be restored."); continue; }
