...
export const CLEANUP_DEFAULTS: CleanupSettings = { focus: "both", location: "vault", exact: true, dominance: true, preferences: false, aggressive: false, sources: ["voltron"], priorities: { health: 1, melee: 1, grenade: 1, super: 1, class: 1, weapons: 1 } };
...
function comparison(candidate: Item, keeper: Item, settings: CleanupSettings, sources: CleanupWishlist[]): { confidence: 99 | 95 | 70; reason: string } | undefined {
  if (family(candidate) !== family(keeper) || keeper.power < candidate.power || (candidate.masterworked && !keeper.masterworked)) return;
  if (weapon(candidate) && weapon(keeper)) {
    ...
    if (roll(candidate) !== roll(keeper)) {
      ...
      if (settings.aggressive && settings.preferences && sources.length === new Set(settings.sources).size && sources.every((source) => modes.every((mode) => endorsed(keeper, source, mode) === true && endorsed(candidate, source, mode) === false))) return { confidence: 70, reason: `Selected sources prefer an available roll on the kept copy for ${settings.focus}. This is a preference, not proof this roll is bad.` };
      return;
    }
  ...
  if (settings.aggressive && settings.preferences && stats.some((key) => settings.priorities[key] > 0) && stats.every((key) => !settings.priorities[key] || keeper.baseStats[key] >= candidate.baseStats[key]) && stats.some((key) => settings.priorities[key] > 0 && keeper.baseStats[key] > candidate.baseStats[key])) return { confidence: 70, reason: "Same armor fit; the kept copy improves your chosen stats. Ignored stats may be worse." };
}
...
export function analyzeCleanup(gear: GearData, settings: CleanupSettings, saved: Set<string>, protectionComplete: boolean, sources: CleanupWishlist[] = []): { recommendations: CleanupRecommendation[]; insufficient: string[] } {
  ...
  for (const item of usable) { const key = family(item); const group = families.get(key) || []; group.push(item); families.set(key, group); }
  ...
  for (let i = usable.length - 1; i >= 0; i--) {
    ...
    for (const keeper of families.get(family(item)) || []) {
