...
const filePath = "apps/web/src/pages/Pages.module.css";
...
function selectorUsesUnusedClass(selector) {
...
function transformContainer(source, insideKeyframes = false) {
  let output = "";
  let cursor = 0;
  while (cursor < source.length) {
    ...
    if (insideKeyframes) {
      ...
      const nextBody = nested ? transformContainer(body, false) : body;
      output += `${prelude}{${keyframes ? body : nextBody}}`;
    } else {
      const remaining = splitSelectors(selector).filter((entry) => !selectorUsesUnusedClass(entry));
...
function removeOrphanKeyframes(source) {
  ...
  while (changed) {
    changed = false;
    const matcher = /@(?:-webkit-)?keyframes\s+([-_a-zA-Z][-_a-zA-Z0-9]*)\s*\{/g;
    let match;
    while ((match = matcher.exec(output)) !== null) {
      ...
      const references = [...output.matchAll(new RegExp(`\\b${escaped}\\b`, "g"))].length;
      if (references === 1) {
...
const after = removeOrphanKeyframes(transformContainer(before));
if (after === before) throw new Error("No final unused CSS selectors were removed");
fs.writeFileSync(absolute, after);
console.log(`Removed ${Buffer.byteLength(before) - Buffer.byteLength(after)} bytes of verified unused shared-page CSS.`);
